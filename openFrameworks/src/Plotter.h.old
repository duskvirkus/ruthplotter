...
private:
  ofSerial serial;
  bool ready;
  deque<vector<byte>> codeQueue;

  void start();

  void send(vector<byte>);
  void addCommand(vector<byte>, std::string);
  void addPadding(vector<byte>, int);
  void addUInt16(vector<byte>, u_int16_t);
  void addFloat32(vector<byte>, float);
};
...
//   void move(PVector v) { move(v.x, v.y); }
...
//   void mark(PVector v) { mark(v.x, v.y); }
...
//   int width() { return 2870; }

//   int height() { return 2030; }
...
//     byte[] bytes = new byte[]{(byte)(a & 0xff), (byte)((a >> 8) & 0xff)};
